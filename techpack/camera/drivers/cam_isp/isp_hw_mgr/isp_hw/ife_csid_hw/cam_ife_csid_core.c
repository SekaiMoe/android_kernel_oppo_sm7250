...
#ifdef VENDOR_EDIT
	CAM_INFO(CAM_ISP, "CSID:%d De-init CSID HW",
		csid_hw->hw_intf->hw_idx);
#endif
	...
	void *release_args, uint32_t arg_size)
{
	...
	if ((res->res_state <= CAM_ISP_RESOURCE_STATE_AVAILABLE) ||
		...
		CAM_WARN(CAM_ISP,
			...
			res->res_state);
#ifdef VENDOR_EDIT
/*shitianyu@camera modify to fix case 4329446 for monkey*/
		if (res->res_type != CAM_ISP_RESOURCE_CID) {
			goto end;
		} else {
			cid_data = (struct cam_ife_csid_cid_data    *) res->res_priv;
	...
	struct cam_ife_csid_hw *csid_hw, bool reset_hw)
{
	...
	if (rc <= 0) {
#ifdef VENDOR_EDIT
        CAM_INFO(CAM_ISP, "CSID:%d csid_reset fail rc = %d, continuing",
