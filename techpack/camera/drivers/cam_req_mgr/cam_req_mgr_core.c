...
#ifdef VENDOR_EDIT
// fuyipeng@camera modify the log level
			CAM_DBG(CAM_CRM,
...
#else
/*wangjingkai@camera add for super night recovery error*/
...
#ifdef VENDOR_EDIT //20191121, shubham.makrariya@camera, CRM recovery failed issue QC case:04310045
...
#ifdef VENDOR_EDIT
// fuyipeng@camera modify the CRM issue case:04245952
	/*
	...
	struct cam_req_mgr_slot *slot)
{
	...
	CAM_DBG(CAM_CRM, "my ts %lld other ts %lld delta %lld ms_diff %lld",
		link->sof_timestamp, sync_link->sof_timestamp,
	...
	if ((sync_link->sof_timestamp > 0) &&
		...
		if (req_id > sync_req_id) {
			...
			(sync_rd_slot->sync_mode == CAM_REQ_MGR_SYNC_MODE_SYNC)) {
			/* I am leader */
			CAM_DBG(CAM_CRM, "leader link %x my ts %lld other %lld", link->link_hdl,
					link->sof_timestamp, sync_link->sof_timestamp);
#endif
...
{
#ifndef VENDOR_EDIT //20191121, shubham.makrariya@camera, CRM recovery failed issue QC case:04310045
