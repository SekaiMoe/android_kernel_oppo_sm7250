...
#ifdef VENDOR_EDIT
// fuyipeng@camera modify for sigbrot result case04254263
	...
	struct v4l2_event *new)
{
	...
	case V4L_EVENT_CAM_REQ_MGR_SOF:
                CAM_ERR_RATE_LIMIT(CAM_CRM, "Failed to notify SOF event");
                CAM_INFO_RATE_LIMIT(CAM_CRM,
                        "SOF Sess %X FrameId %lld ReqId %lld link %X",
	...
	case V4L_EVENT_CAM_REQ_MGR_SOF_BOOT_TS:
                CAM_ERR_RATE_LIMIT(CAM_CRM, "Failed to notify BOOT_TS event");
                CAM_INFO_RATE_LIMIT(CAM_CRM,
                "TS Sess %X FrameId %lld ReqId %lld link %X",
			ev_header->session_hdl,
			ev_header->u.frame_msg.frame_id,
			ev_header->u.frame_msg.request_id,
			ev_header->u.frame_msg.link_hdl);
