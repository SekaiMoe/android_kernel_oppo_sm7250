...
static ssize_t flash_on_off(struct cam_flash_ctrl *flash_ctrl)
{
	...
	switch (flash_mode)
	{
		...
		case 1:
...
#else
			if (strcmp(vendor_flash_ctrl->flash_name,"pmic_19125") == 0) {
				flash_data.led_current_ma[0] = 107;
				flash_data.led_current_ma[1] = 107;
			} else if((strcmp(vendor_flash_ctrl->flash_name,"pmic_19015") == 0) ||
				(strcmp(vendor_flash_ctrl->flash_name,"pmic_19191") == 0)) {
			...
			CAM_ERR(CAM_FLASH, "flash current: %d", flash_data.led_current_ma[0]);
#endif
