...
#ifdef VENDOR_EDIT
#define PRJ_VERSION_PATH  "/proc/oplusVersion/prjName"
static bool actuator_power_wide = 0;
static char strPrjname[10] = {0};
...
int RamWriteByte(struct camera_io_master *io_master_info,
		uint32_t addr, uint32_t data, unsigned short mdelay)
{
	...
	for(i = 0; i < retry; i++)
	{
		rc = camera_io_dev_write(io_master_info, &i2c_write);
		if (rc < 0) {
...
static int32_t cam_actuator_update_pid(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	CAM_INFO(CAM_ACTUATOR, "entry imx586 new actuator.");
	if (0x5A >> 1 == a_ctrl->io_master_info.cci_client->sid) {
		...
		for (cnt = 0; cnt < UPDATE_REG_SIZE; cnt++) {
			rc = RamWriteByte(&(a_ctrl->io_master_info),
					update_reg_arr[cnt][0],
					update_reg_arr[cnt][1],
					update_reg_arr[cnt][2]);
			if (rc < 0){
				CAM_ERR(CAM_ACTUATOR, "write imx586 new PID data error in step %d:rc %d", cnt, rc);
			}
		}
		for (re = 0; re < retry; re++){
			...
			if (rc < 0 || ((reg_data & 0x04) != 0)) {
				...
				rc = RamWriteByte(&(a_ctrl->io_master_info), update_reg_arr[16][0], update_reg_arr[16][1], update_reg_arr[16][2]);
				rc = RamWriteByte(&(a_ctrl->io_master_info), update_reg_arr[17][0], update_reg_arr[17][1], update_reg_arr[17][2]);
				rc = RamWriteByte(&(a_ctrl->io_master_info), update_reg_arr[18][0], update_reg_arr[18][1], update_reg_arr[18][2]);
			}else {
				break;
			}
		}
		rc = RamWriteByte(&(a_ctrl->io_master_info), 0xAE, 0x00, 0);
		if (rc < 0){
	...
	if (0x5A >> 1 == a_ctrl->io_master_info.cci_client->sid) {
		...
		for (cnt = 1; cnt < UPDATE_REG_SIZE; cnt++) {
			rc = camera_io_dev_read(
				...
				CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE);
			if (rc < 0){
				CAM_ERR(CAM_ACTUATOR, "read imx586 new PID data error in step %d:rc %d", cnt, rc);
				break;
			}
			if (reg_data != update_reg_arr[cnt][1]){
				CAM_ERR(CAM_ACTUATOR, "imx586 new PID data wrong in step %d:rc %d, reg_data is %x", cnt, rc,reg_data);
				rc = -1;
				break;
			}
		}
...
static int32_t cam_actuator_power_up(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	struct cam_sensor_power_ctrl_t *power_info;
#ifdef VENDOR_EDIT
	struct camera_io_master m_io_master_info;
	struct cam_sensor_cci_client m_cam_sensor_cci_client;
#endif
	...
	if (0 == getProject(strPrjname)) {
		if (0 == strcmp(strPrjname,"19191") || 0 == strcmp(strPrjname,"19192") || 0 == strcmp(strPrjname,"19591")) {
			if (1 == a_ctrl->io_master_info.cci_client->cci_i2c_master && 0 == a_ctrl->io_master_info.cci_client->cci_device) {
				actuator_power_wide = 1;   //imx 708 actuator_power_up
			}
			else if (0 == a_ctrl->io_master_info.cci_client->cci_i2c_master && 1 == a_ctrl->io_master_info.cci_client->cci_device) {
				if (actuator_power_wide == 0) {
					m_io_master_info.cci_client = &m_cam_sensor_cci_client;
					m_io_master_info.master_type                = a_ctrl->io_master_info.master_type;
					m_io_master_info.cci_client->cci_device     = 0;
					m_io_master_info.cci_client->freq           = a_ctrl->io_master_info.cci_client->freq;
					m_io_master_info.cci_client->i2c_freq_mode  = a_ctrl->io_master_info.cci_client->i2c_freq_mode;
					m_io_master_info.cci_client->cci_i2c_master = 1;
					m_io_master_info.cci_client->sid            = a_ctrl->io_master_info.cci_client->sid;
					m_io_master_info.cci_client->cid            = a_ctrl->io_master_info.cci_client->cid;
					m_io_master_info.cci_client->timeout        = a_ctrl->io_master_info.cci_client->timeout;
					m_io_master_info.cci_client->retries        = a_ctrl->io_master_info.cci_client->retries;
					m_io_master_info.cci_client->id_map         = a_ctrl->io_master_info.cci_client->id_map;

					rc = camera_io_init(&m_io_master_info);
					if (rc < 0)
						CAM_ERR(CAM_ACTUATOR, "wide cci init failed: rc: %d", rc);

					rc = RamWriteByte(&m_io_master_info, 0x02, 0x20, 0x00); //& 0x02

					if (rc < 0)
						CAM_ERR(CAM_ACTUATOR,"write the 0x02 failed ,rc = %d", rc);

					rc = camera_io_release(&m_io_master_info);
					if (rc < 0)
						CAM_ERR(CAM_ACTUATOR, "wide cci release failed: rc: %d ", rc);
				}
			}
			if(0 == a_ctrl->io_master_info.cci_client->cci_i2c_master && 0 == a_ctrl->io_master_info.cci_client->cci_device){
				for (re = 0;re < retry; re++){
...
static int32_t cam_actuator_power_down(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (0 == strcmp(strPrjname,"19191") || 0 == strcmp(strPrjname,"19192") || 0 == strcmp(strPrjname,"19591")) {
		if (1 == a_ctrl->io_master_info.cci_client->cci_i2c_master && 0 == a_ctrl->io_master_info.cci_client->cci_device) {
			actuator_power_wide = 0; //imx 708 actuator_power_down
		}
	}
#endif
