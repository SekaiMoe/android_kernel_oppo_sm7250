...
#ifdef VENDOR_EDIT
/*shitianyu@camera modified for bug 2485624*/
		...
		unsigned long min_usecs, unsigned long max_usecs)
{
	...
	if (cnt > retry) {
		CAM_INFO(CAM_CSIPHY, "Poll failed by value: %u",tmp);
			rc = -EINVAL;
	}

	CAM_DBG(CAM_CSIPHY, "POLL - value: %d rc: %d", tmp, rc);
	return rc;
}
#endif
...
void cam_csiphy_reset(struct csiphy_device *csiphy_dev)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (csiphy_dev->ctrl_reg->csiphy_reset_reg[i].csiphy_param_type
							== CSIPHY_RESET_POLL_REG) {
			int poll_retval = -1;
			poll_retval = cam_io_poll_value_masked(csiphy_dev->ctrl_reg->csiphy_reset_reg[i].reg_addr + base,
					...
					2000);
			if (poll_retval < 0) {
				CAM_ERR(CAM_CSIPHY,"cam csiphy reset poll failed");
			} else {
				 CAM_DBG(CAM_CSIPHY, "Reset successfull ");
			}
			continue;
		}
#endif
...
#ifdef VENDOR_EDIT
	CAM_DBG(CAM_CSIPHY, "PHY reset exit index : %d",csiphy_dev->soc_info.index);
#endif
#ifdef VENDOR_EDIT
	/*shitianyu@camera modified for bug 2485624*/
...
int32_t cam_csiphy_config_dev(struct csiphy_device *csiphy_dev)
{
	...
	for (i = 0; i < size; i++) {
		...
		case CSIPHY_3PH_REGS:
			...
			break;
#ifdef VENDOR_EDIT
		case CSIPHY_RESET_POLL_REG:
			poll_retval = cam_io_poll_value_masked(csiphy_common_reg->reg_addr + csiphybase,
 					csiphy_common_reg->reg_data,
					csiphy_common_reg->reg_data_mask,
					5,
					1000,
					2000);
			if (poll_retval < 0) {
			...
			void *arg)
{
...
#ifdef VENDOR_EDIT
		if(soc_info != NULL && csiphy_dev != NULL) {
		    CAM_INFO(CAM_CSIPHY, "START DEV rshankar PHY NUM %d state = %d", soc_info->index, csiphy_dev->csiphy_state);
		} else {
		    CAM_INFO(CAM_CSIPHY, "START DEV rshankar soc_info is NULL");
		}
#endif
	...
	case CAM_STOP_DEV: {
...
#ifdef VENDOR_EDIT
		if(soc_info != NULL && csiphy_dev != NULL){
		    CAM_INFO(CAM_CSIPHY, "STOP DEV rshankar PHY NUM %d state = %d", soc_info->index, csiphy_dev->csiphy_state);
		} else {
		    CAM_INFO(CAM_CSIPHY, "STOP DEV rshankar soc_info is NULL");
		}
#endif
		...
		if (rc < 0)
			...
			CAM_INFO(CAM_CSIPHY, "Disable hardware done");
#endif
...
#ifdef VENDOR_EDIT
		if(soc_info != NULL && csiphy_dev != NULL) {
		    CAM_INFO(CAM_CSIPHY, "CAM_RELEASE_DEV rshankar PHY NUM %d state = %d", soc_info->index, csiphy_dev->csiphy_state);
		} else {
		    CAM_INFO(CAM_CSIPHY, "CAM_RELEASE_DEV rshankar soc_info is NULL");
		}
	...
	case CAM_START_DEV: {
		...
		if (rc != 0) {
			...
			CAM_INFO(CAM_CSIPHY, "enable hw done");
		}
#endif
		...
		if (rc < 0) {
			...
			CAM_INFO(CAM_CSIPHY, "phy config done");
		}
		csiphy_dev->start_dev_count++;
