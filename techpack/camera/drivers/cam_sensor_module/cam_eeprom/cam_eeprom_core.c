	...
	for (j = 0; j < block->num_map; j++) {
		...
		for (retry = 3; retry > 0 && rc < 0; retry--) {
			rc = camera_io_dev_read_seq(&e_ctrl->io_master_info,
				...
				emap[j].mem.data_type,
				emap[j].mem.valid_size);
			CAM_ERR(CAM_EEPROM,"Try camera_io_dev_read_seq again : retry=%d,rc=%d",retry,rc);
		}
#endif
			if (rc) {
	...
	void *arg)
{
	...
	CAM_ERR(CAM_EEPROM, "write_eeprom, cam: ID: %x, addr: %x, DataSize: %d, isWRP: %x",
	...
	CAM_ERR(CAM_EEPROM, "write start, cam: ID: 0x%x, reg_addr: 0x%x, size: %d",
	...
	if (((cam_write_eeprom.cam_id == 0x01) || (cam_write_eeprom.cam_id==0x02) || (cam_write_eeprom.cam_id==0x03)  || (cam_write_eeprom.cam_id==0x04) || (cam_write_eeprom.cam_id==0x05))) {
		for (i = 0; i < cam_write_eeprom.calibDataSize;) {
			...
			for (j = 0; (j < WRITE_DATA_MAX_LENGTH && i < cam_write_eeprom.calibDataSize); j++) {
				i2c_reg_arrays[j].reg_addr = cam_write_eeprom.baseAddr + i;
	...
	void *arg)
{
	...
	if (check_eeprom_data.cam_id == 0x01 || check_eeprom_data.cam_id == 0x02  || check_eeprom_data.cam_id == 0x03 || check_eeprom_data.cam_id == 0x04 || check_eeprom_data.cam_id == 0x05) {
