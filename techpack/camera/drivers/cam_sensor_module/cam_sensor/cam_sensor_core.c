...
#define PCB_VERSION_PATH  "/proc/oplusVersion/pcbVersion"
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	const uint8_t IMX586_EEPROM_MODID_ADDR = 0x00;
	const uint8_t IMX586_FIRST_SOURCE_VCMID = 0xC2;
	const uint8_t IMX586_SECOND_SOURCE_VCMID = 0x3A;
	const uint8_t IMX586_FIRST_SOURCE_VCMID_19191 = 0x3A;
	const uint8_t IMX586_SECOND_SOURCE_VCMID_19191 = 0xC2;
	/*add by Fangyan @ Cam.Drv 20200312, for distinguish the different module, need to delete when 19015 evt*/
	const uint8_t IMX586_THIRD_SOURCE_VAFID = 0x1C;
	const uint8_t IMX586_SECOND_SOURCE_MODID = 0x03;
	const uint32_t IMX586_FIRST_SOURCE_CHIPID = 0xFFFF;
	const uint32_t IMX586_SECOND_SOURCE_CHIPID = 0xFFFE;
	const uint32_t IMX586_FIRST_SOURCE_CHIPID_19191 = 0x586;
	const uint32_t IMX586_SECOND_SOURCE_CHIPID_19191 = 0xFFFF;
	static char strPrjname[10] = {0};

	if(0 == strPrjname[0] && 0 == getProject(strPrjname)) {
	...
	rc = camera_io_dev_read(
		...
		else if (slave_info->sensor_id == 0xF2d0)
		{
			gc02m0_high = slave_info->sensor_id_reg_addr & 0xff00;
			gc02m0_high = gc02m0_high >> 8;
			gc02m0_low = slave_info->sensor_id_reg_addr & 0x00ff;
...
/*add by Fangyan @ Cam.Drv 20200317,for force the replace module not to probe the normal bin*/
		else if (((slave_info->sensor_id == 0x586) || (slave_info->sensor_id == 0xFFFF))
			&& (0 == strcmp(strPrjname,"19015") || 0 == strcmp(strPrjname,"19016")
			|| 0 == strcmp(strPrjname,"19525") || 0 == strcmp(strPrjname,"19517")
			|| 0 == strcmp(strPrjname,"19518"))) {
			...
			rc = cam_cci_i2c_read(&ee_cci_client,
				 ...
				 CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = cam_cci_i2c_read(&ee_cci_client,
				 IMX586_EEPROM_MODID_ADDR,
				 &ee_modid, CAMERA_SENSOR_I2C_TYPE_WORD,
				 CAMERA_SENSOR_I2C_TYPE_BYTE);

			CAM_INFO(CAM_SENSOR, "distinguish imx586 camera module,modId:0x%x vcm id : 0x%x ",ee_modid,ee_vcmid);
			if (IMX586_THIRD_SOURCE_VAFID == ee_vcmid){
				chipid = IMX586_FIRST_SOURCE_CHIPID;
			}
			else if (ee_modid == IMX586_SECOND_SOURCE_MODID) {
				chipid = IMX586_FIRST_SOURCE_CHIPID;
			}
			else
			{
			...
			CAM_INFO(CAM_SENSOR, "distinguish imx586 camera module,sensor_id_reg_addr:0x%x chipid: 0x%x ",slave_info->sensor_id_reg_addr,chipid);

		}
		else if (slave_info->sensor_id == IMX586_FIRST_SOURCE_CHIPID || \
...
/*add by Fangyan @ Cam.Drv 20200312, for distinguish the different module, need to delete when 19015 evt*/
			if (IMX586_FIRST_SOURCE_VCMID == ee_vcmid) {
				chipid = IMX586_FIRST_SOURCE_CHIPID;
			} else if (IMX586_SECOND_SOURCE_VCMID == ee_vcmid || IMX586_THIRD_SOURCE_VAFID == ee_vcmid) {
				chipid = IMX586_SECOND_SOURCE_CHIPID;
			}else{
				chipid = IMX586_FIRST_SOURCE_CHIPID;
			}
		}
		else if ((slave_info->sensor_id == 0xFFFF || slave_info->sensor_id == 0x586) &&
				 (0 == strcmp(strPrjname,"19191") || 0 == strcmp(strPrjname,"19192") || 0 == strcmp(strPrjname,"19591"))) {
			memcpy(&ee_cci_client, s_ctrl->io_master_info.cci_client,
							sizeof(struct cam_sensor_cci_client));
			ee_cci_client.sid = IMX586_EEPROM_SID;
			rc = cam_cci_i2c_read(&ee_cci_client,
			...
			CAM_ERR(CAM_SENSOR, "distinguish imx586 camera module, vcm id : 0x%x ",ee_vcmid);
			if (IMX586_FIRST_SOURCE_VCMID_19191 == ee_vcmid) {
				chipid = IMX586_FIRST_SOURCE_CHIPID_19191;
			} else if (IMX586_SECOND_SOURCE_VCMID_19191 == ee_vcmid) {
				chipid = IMX586_SECOND_SOURCE_CHIPID_19191;
			}else{
				chipid = IMX586_FIRST_SOURCE_CHIPID_19191;
			}
		}
...
#ifdef VENDOR_EDIT
/*shitianyu@camera modify to fix case 04326266 [SM7250][camera][monkey]Acquire sensor resource failed caused sigabort from QC patch*/
		if(s_ctrl->sensor_state == CAM_SENSOR_ACQUIRE) {
		  CAM_WARN(CAM_SENSOR,
				"Already in Acquire state %d",
				s_ctrl->sensor_state);
				goto release_mutex;
		}
#endif
...
#ifdef VENDOR_EDIT
		CAM_INFO(CAM_SENSOR,
					"CAM_ACQUIRE_DEV cmd->reserved is %d ",cmd->reserved);

		if (!cmd->reserved) {
			if (s_ctrl->soc_info.index != 2 && !(s_ctrl->soc_info.index == 4 &&
				(0 == strcmp(strPrjname,"19015") || 0 == strcmp(strPrjname,"19016")
				|| 0 == strcmp(strPrjname,"19525") || 0 == strcmp(strPrjname,"19517")
				|| 0 == strcmp(strPrjname,"19518")))){
				tof_oem_start();
