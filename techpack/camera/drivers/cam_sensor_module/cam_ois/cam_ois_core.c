...
#ifdef VENDOR_EDIT
static int cam_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl, uint32_t gyro_type)
#else
...
#ifdef VENDOR_EDIT
/*lixin add for camera_sec ois at 2019/09/24*/
	uint32_t ee_vcmid = 0 ;
...
/*lixin add for camera_sec ois at 2019/09/24*/
	...
	if (IMX586_SECOND_SOURCE_VCMID == ee_vcmid) {
		snprintf(name_coeff, 64, "%s_sec.coeff", o_ctrl->ois_name);
		snprintf(name_prog, 64, "%s_sec.prog", o_ctrl->ois_name);
	}

	//if gyro_type == 1  gyroname is lsm6dsm else use the first source
	//else if gyro_type == 2  gyroname is bmi260 else use the first source
	if (gyro_type == 1 &&
		strcmp(o_ctrl->ois_name, "ois_bu63169_19191") == 0)
	{
		snprintf(name_coeff, 64, "ois_bu63169_sec.coeff");
		snprintf(name_prog, 64, "ois_bu63169_sec.prog");
		CAM_INFO(CAM_SENSOR, "using the first source firmware gyro on lsm6dsm");
	}
	else if (gyro_type == 1 &&
		strcmp(o_ctrl->ois_name, "ois_bu63169_19191_sec") == 0)
	{
		snprintf(name_coeff, 64, "ois_lsm6dsm_bu63169_19191_sec.coeff");
		snprintf(name_prog, 64, "ois_lsm6dsm_bu63169_19191_sec.prog");
		CAM_INFO(CAM_SENSOR, "using the second source firmware gyro on lsm6dsm");
	}
	else if (gyro_type == 2 &&
		strcmp(o_ctrl->ois_name, "ois_bu63169_19191") == 0)
	{
		snprintf(name_coeff, 64, "ois_bmi260_bu63169_19191.coeff");
		snprintf(name_prog, 64, "ois_bmi260_bu63169_19191.prog");
		CAM_INFO(CAM_SENSOR, "using the first source firmware gyro on bmi260");
	}
	else if (gyro_type == 2 &&
		strcmp(o_ctrl->ois_name, "ois_bu63169_19191_sec") == 0)
	{
		snprintf(name_coeff, 64, "ois_bmi260_bu63169_19191_sec.coeff");
		snprintf(name_prog, 64, "ois_bmi260_bu63169_19191_sec.prog");
		CAM_INFO(CAM_SENSOR, "using the second source firmware gyro on bmi260");
	}
	else
	{
		CAM_INFO(CAM_SENSOR, "using the first source gyro");
	}
#endif
...
#ifdef VENDOR_EDIT
			rc = cam_ois_fw_download(o_ctrl,dev_config.gyro_type);
#else
